...
## Agents « logiciels » : ils GÉNÈRENT des scripts ; l'exécution est optionnelle (locale ou distante)

Les agents créatifs (04 Blender, 05 Unreal, 06 GIMP/Krita/…, 07 FFmpeg, 08 son) produisent des **scripts/fichiers texte** via LLM, sauvegardés dans `output/`, puis affichés comme **commandes headless** prêtes à copier-coller (`utils_headless.py`). Par défaut rien ne s'exécute. Deux exécuteurs **mutuellement exclusifs** peuvent lancer les rendus, tous deux sur la **même liste blanche stricte** `OUTILS_STUDIO` (blender/unreal/ffmpeg/csound) et la même `construire_commande` (jamais construite par l'IA, jamais via shell) :
- `--worker` → `ExecuteurDistant` (client_worker.py) sur une machine de rendu distante ;
- `--local` → `ExecuteurLocal` (executeur_local.py) **sur cette machine** (headless), même interface publique et même contrat de retour que l'exécuteur distant. Il n'existe toujours **aucun** pilotage souris/clavier/GUI.

**Décision de direction (importante) :** le projet est passé de « rien ne s'exécute en local, seulement via worker » à « exécution locale autonome **opt-in** via `--local` ». **Why:** l'utilisateur a explicitement demandé que l'agent lance lui-même les logiciels. Le compromis assumé : exécuter en local un script Blender/Unreal/FFmpeg généré par IA = **exécution de code arbitraire** sur la machine de l'utilisateur (Csound `.csd` est nettement moins risqué). D'où : `--local` opt-in explicite + avertissement clair au démarrage ; les garde-fous restent ceux du worker (liste blanche fermée, commande maison sans shell, nom de fichier assaini, dossier isolé, délai max, échoue-fermé). La source unique du nom de fichier sûr est `worker_distant.nom_script_securise` (partagée worker + local pour éviter toute divergence de sécurité).

**How to apply:** nouvel agent « logiciel » = choisir un outil **réellement headless piloté par script** (ex. Csound `.csd` : orchestre+partition dans un fichier texte, `csound f.csd -o out.wav`). Le brancher : schéma dans `shared_state.py` (+ validateur), `Etape` `critique=False` dans `construire_pipeline`, callback `_enregistrer_*`, commande dans `utils_headless.py`. Pour l'exécution : ajouter l'outil à `OUTILS_STUDIO`, `ConfigWorker`, `outils_disponibles`, `construire_commande`, l'extension de fichier ; puis une méthode `executer_<outil>` dans `ExecuteurDistant` ET `ExecuteurLocal` (même contrat de dict `{travail_id,code_retour,duree,journal_path,nb_fichiers,dossier_fichiers}` pour réutiliser `_enregistrer_rendu`/`_afficher_rendu`), + une étape dans `etapes_rendu_distant` ET `etapes_rendu_local`. En local, `dossier_fichiers` sert aussi de dossier de chaînage (FFmpeg s'exécute dans le dossier du rendu Blender via `rendu_blender_fichiers`). **Invariant sécurité :** le chaînage local n'accepte QUE des dossiers situés sous la racine des rendus locaux (`ExecuteurLocal._chainage_autorise`, realpath + anti-`../`) — le chemin vient de `world_state.json` et ne doit jamais faire exécuter/écrire un rendu hors périmètre s'il est falsifié (équivalent du worker qui ne chaîne que sur un identifiant de son registre).
